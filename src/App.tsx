            ...
            {isEditing ? (
              <>
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  <input
                    value={p.number || ''}
                    onChange={e => updatePlayerField(p.id, 'number', e.target.value)}
                    placeholder="#"
                  ...
                  {['Fr', 'So', 'Jr', 'Sr'].map(grade => (
                    <button key={grade} onClick={() => updatePlayerField(p.id, 'grade', p.grade === grade ? null : grade)}
                      style={{
                        ...
                        background: p.grade === grade ? COLORS.gold : COLORS.navyDark,
                        color: p.grade === grade ? COLORS.textDark : COLORS.muted,
                        border: `1px solid ${p.grade === grade ? COLORS.gold : COLORS.border}`,
                      }}>
                      {grade}
                    </button>
                  ))}
...
function CourtBackground() {
  ...
  const midX = W / 2;
  const midY = H / 2;
  const keyW = courtH * 0.33;
  const keyD = courtW * 0.175;
  const ftR = keyW * 0.52;
