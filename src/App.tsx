...
function PlayerDashboard({ playerRecord, team }) {
  ...
  const playerAggs = allPlayers.map(p => {
    ...
    games.forEach(g => {
      ...
      if (!s) return;
      pgp++; ppts += calcPts(s); peff += calcEff(s);
    });
  ...
  const tabBtn = (key, label) => (
    <button onClick={() => setTab(key)} style={{
      flex: 1, padding: '7px 2px', fontWeight: tab === key ? 800 : 600, fontSize: 11,
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.navyDark, color: COLORS.text, fontFamily: 'sans-serif' }}>
      ...
      <div style={{ padding: '16px 20px', maxWidth: 600, margin: '0 auto' }}>
        ...
        {tab === 'schedule' && (
          <div>
            {seasons.map(s => <SeasonViewReadOnly key={s.id} season={s} team={team} COLORS={COLORS} />)}
          </div>
        )}
...
function ScoutViewReadOnly({ team, COLORS }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setSessions(sess || []); setOppPlayers(players || []);
    });
  }, [selectedOpp]);
  ...
  if (selectedOpp) {
    const playerAggs = oppPlayers.map(p => {
      ...
      sessions.forEach(sess => {
        ...
        if (!st) return;
        gp++;
        Object.keys(st).forEach(k => { totals[k] = (totals[k] || 0) + (st[k] || 0); });
      });
      if (gp === 0) return null;
      const pts = ((totals['2PM'] || 0) * 2) + ((totals['3PM'] || 0) * 3) + (totals['FTM'] || 0);
      const eff = pts + (totals['O'] || 0) + (totals['D'] || 0) + (totals['AST'] || 0) + (totals['STL'] || 0) + (totals['BS'] || 0)
        - (totals['TO'] || 0) - (totals['PF'] || 0) - (totals['2PA'] || 0) - (totals['3PA'] || 0) - (totals['FTA'] || 0);
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        {invites.length === 0 ? (
          ...
          <div key={inv.id} style={{ background: COLORS.navyMid, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: '12px 14px', marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 10 }}>Requested to join with team code · {new Date(inv.created_at).toLocaleDateString()}</div>
            <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 6 }}>Link to roster player:</div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => approve(inv)}
                style={{ flex: 1, padding: 10, background: COLORS.green, border: 'none', borderRadius: 8, color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
                ✓ Approve
              </button>
              <button onClick={() => deny(inv)}
                style={{ flex: 1, padding: 10, background: 'none', border: `1px solid ${COLORS.red}`, borderRadius: 8, color: COLORS.red, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                ✗ Deny
              </button>
            </div>
          </div>
...
// ── Scout components (keeping from previous version) ──────────────────────────
function ScoutFilmTagger({ opponent, team, onClose, onSaved }) {
  ...
  const calcScoutEff = (s) => {
    const pos = calcScoutPts(s) + (s['O'] || 0) + (s['D'] || 0) + (s['AST'] || 0) + (s['STL'] || 0) + (s['BS'] || 0) + (s['DF'] || 0) + (s['CHG_taken'] || 0);
          ...
          {players.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => setSelectedPlayer(sel ? null : p)}
                style={{ padding: '7px 10px', borderRadius: 8, border: sel ? `2px solid ${COLORS.gold}` : `1px solid ${COLORS.border}`, background: sel ? COLORS.goldLight : COLORS.navyMid, color: sel ? COLORS.textDark : COLORS.text, cursor: 'pointer' }}>
...
{"gameDate":"<YYYY-MM-DD or null>","players":[{"number":"<digits>","name":"<name>","pts":<n>,"fgm":<n>,"fga":<n>,"fg3m":<n>,"fg3a":<n>,"ftm":<n>,"fta":<n>,"oreb":<n>,"dreb":<n>,"ast":<n>,"defl":<n>,"stl":<n>,"blk":<n>,"to":<n>,"pf":<n>,"chg":<n>,"mins":<n>}]}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        {step === 'upload' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, paddingTop: 32 }}>
            ...
            <div style={{ color: COLORS.muted, fontSize: 12, textAlign: 'center', maxWidth: 280, lineHeight: 1.5 }}>Upload a Hudl PDF from any game {opponent.name} played.</div>
...
function ScoutReport({ opponent, team, onClose }) {
  ...
  const playerAggregates = oppPlayers.map(p => {
    ...
    sessions.forEach(sess => {
      ...
      if (!st) return;
      gp++;
      KEYS.forEach(k => { totals[k] += st[k] || 0; });
      totalMins += st.mins || 0;
    });
    if (gp === 0) return null;
    ...
    return {
      player: p, gp, ppg: pts/gp, epg: eff/gp, rpg: reb/gp, apg: totals['AST']/gp, per,
      spg: totals['STL']/gp, topg: totals['TO']/gp,
      fg2pct: fg2a > 0 ? Math.round((fg2m/fg2a)*100) : 0,
      fg3pct: fg3a > 0 ? Math.round((fg3m/fg3a)*100) : 0,
      ftpct: fta > 0 ? Math.round((ftm/fta)*100) : 0,
      fgma: `${fg2m+fg3m}/${fg2a+fg3a}`, fg3ma: `${fg3m}/${fg3a}`, ftma: `${ftm}/${fta}`,
    };
  }).filter(Boolean).sort((a, b) => b.epg - a.epg);
                ...
                <thead>
                  <tr style={{ background: '#1a3a6b', color: '#fff' }}>
                    {['#','Player','POS','GP','PPG','FGM/A','FG%','3PM/A','3P%','FTM/A','FT%','RPG','APG','SPG','TOV','EFF','PER'].map(h => (
                      <th key={h} style={{ padding: '7px 6px', textAlign: h === '#' || h === 'Player' || h === 'POS' ? 'left' : 'center', color: h === 'EFF' || h === 'PER' ? '#c8a84b' : '#fff' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
...
function TeamViewInner({ team, onBack }) {
  ...
  const tabBtn = (key, label) => (
    <button onClick={() => setTab(key)} style={{
      padding: '7px 2px', fontWeight: tab === key ? 'bold' : 'normal', fontSize: 11, lineHeight: 1.2,
      background: tab === key ? COLORS.gold : 'rgba(255,255,255,0.08)',
      color: tab === key ? COLORS.textDark : COLORS.text,
      border: `1px solid ${tab === key ? COLORS.gold : COLORS.border}`,
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.navyDark, color: COLORS.text, fontFamily: 'sans-serif' }}>
      {showPendingApprovals && <PendingApprovalsScreen team={team} onClose={() => { setShowPendingApprovals(false); supabase.from('player_invites').select('id', { count: 'exact' }).eq('team_id', team.id).eq('status', 'pending').then(({ count }) => setPendingCount(count || 0)); }} />}
      <div style={{ background: COLORS.navy, borderBottom: `3px solid ${COLORS.gold}`, padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ marginLeft: productLogo ? 0 : 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
          {isCoach && (
            <button onClick={() => setShowPendingApprovals(true)}
              style={{ position: 'relative', padding: '7px 12px', background: pendingCount > 0 ? 'rgba(200,168,75,0.15)' : 'none', border: `1px solid ${pendingCount > 0 ? COLORS.gold : COLORS.border}`, color: pendingCount > 0 ? COLORS.gold : COLORS.muted, borderRadius: 7, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>
...
function App() {
  ...
  try {
    ...
    try {
      // Find team by code
      const { data: themes } = await supabase.from('theme_settings').select('organization_id, theme');
      const matchingTheme = themes?.find(t => t.theme?.teamCode === playerTeamCode.trim());
      ...
      // Find team by org
      const { data: teamData } = await supabase.from('teams').select('id, name').eq('organization_id', matchingTheme.organization_id).limit(1).single();
      ...
      return (
        <div style={{ minHeight: '100vh', background: XOVR.teal, fontFamily: 'sans-serif', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, background: XOVR.teal, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 24, paddingBottom: 32, paddingLeft: 24, paddingRight: 24 }}>
            <div style={{ fontSize: 26, fontWeight: 900, marginBottom: 6, textAlign: 'center', color: XOVR.gold, WebkitTextStroke: '1px #000', textShadow: '2px 2px 0 #000', letterSpacing: 2, fontStyle: 'italic' }}>
              JOIN YOUR TEAM
            </div>
            ...
            <form onSubmit={handlePlayerSignup} style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%', maxWidth: 340 }}>
              <input type="email" placeholder="Your email" value={playerEmail} onChange={e => setPlayerEmail(e.target.value)} required
                style={{ padding: '14px 20px', background: XOVR.inputBg, border: `2px solid ${XOVR.gold}`, borderRadius: 50, color: XOVR.text, fontSize: 14, outline: 'none', width: '100%', boxSizing: 'border-box' }} />
              <input type="password" placeholder="Create a password" value={playerPassword} onChange={e => setPlayerPassword(e.target.value)} required
                style={{ padding: '14px 20px', background: XOVR.inputBg, border: `2px solid ${XOVR.gold}`, borderRadius: 50, color: XOVR.text, fontSize: 14, outline: 'none', width: '100%', boxSizing: 'border-box' }} />
              <input type="text" placeholder="Team code (from your coach)" value={playerTeamCode} onChange={e => setPlayerTeamCode(e.target.value)} required
                style={{ padding: '14px 20px', background: XOVR.inputBg, border: `2px solid ${XOVR.gold}`, borderRadius: 50, color: XOVR.text, fontSize: 14, outline: 'none', width: '100%', boxSizing: 'border-box', textTransform: 'uppercase', letterSpacing: 2 }} />
              <button type="submit" disabled={playerSignupLoading}
                style={{ marginTop: 6, padding: '14px', fontWeight: 900, fontSize: 16, background: XOVR.gold, color: '#000', border: 'none', borderRadius: 50, cursor: 'pointer', letterSpacing: 1, width: '60%', alignSelf: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.35)', opacity: playerSignupLoading ? 0.7 : 1 }}>
                {playerSignupLoading ? 'Sending…' : 'Request Access'}
              </button>
            </form>
            {playerSignupMessage && (
              <p style={{ marginTop: 14, fontSize: 13, color: playerSignupMessage.startsWith('✅') ? XOVR.gold : '#ff6b6b', textAlign: 'center', maxWidth: 300 }}>{playerSignupMessage}</p>
            )}
            <button onClick={() => { setPlayerSignupMode(false); setPlayerSignupMessage(''); }}
    ...
    return (
      <div style={{ minHeight: '100vh', background: XOVR.teal, fontFamily: 'sans-serif', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, background: XOVR.teal, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 28, paddingBottom: 32, paddingLeft: 24, paddingRight: 24 }}>
          ...
          <button onClick={() => setPlayerSignupMode(true)}
            style={{ marginTop: 12, padding: '10px 24px', background: 'none', border: `1px solid ${XOVR.gold}`, color: XOVR.gold, borderRadius: 50, cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>
            🏀 Join as a Player
          </button>
          <div style={{ marginTop: 'auto', paddingTop: 24, fontSize: 10, color: 'rgba(255,255,255,0.2)', letterSpacing: 1 }}>XOVR BASKETBALL © 2026</div>
        </div>
