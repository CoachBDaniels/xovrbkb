...
- fgm/fga = TOTAL field goals including 3s — fga is TOTAL attempts not misses
- fg3m/fg3a = three point makes and TOTAL three point attempts not misses
- ftm/fta = free throw makes and TOTAL free throw attempts not misses
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      matchedStats.forEach(m => {
        ...
        const s = m.hudlStats;
        // 2pt makes and misses (not attempts)
        const fg2m = Math.max(0, (s.fgm || 0) - (s.fg3m || 0));
        const fg2a = Math.max(0, (s.fga || 0) - (s.fg3a || 0));
        const fg2miss = Math.max(0, fg2a - fg2m);
        const fg3miss = Math.max(0, (s.fg3a || 0) - (s.fg3m || 0));
        const ftmiss = Math.max(0, (s.fta || 0) - (s.ftm || 0));

        playerStats[m.rosterPlayerId] = {
          '2PM': fg2m,
          '2PA': fg2miss,  // XOVR stores misses not total attempts
          '3PM': s.fg3m || 0,
          '3PA': fg3miss,  // XOVR stores misses not total attempts
          'FTM': s.ftm || 0,
          'FTA': ftmiss,   // XOVR stores misses not total attempts
          'O': s.oreb || 0,
          'D': s.dreb || 0,
          'AST': s.ast || 0,
          'DF': s.defl || 0,
      ...
      const opp2miss = Math.max(0, opp2a - opp2m);
      const opp3miss = Math.max(0, oppTotals.fg3a - oppTotals.fg3m);
      const oppftmiss = Math.max(0, oppTotals.fta - oppTotals.ftm);

      playerStats['OPP'] = {
        '2PM': opp2m, '2PA': opp2miss,
        '3PM': oppTotals.fg3m, '3PA': opp3miss,
        'FTM': oppTotals.ftm, 'FTA': oppftmiss,
