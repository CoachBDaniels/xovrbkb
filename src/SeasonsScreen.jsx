...
              style={{ flex: 1, padding: 8, background: COLORS.navyDark, border: `1px solid ${COLORS.border}`, borderRadius: 7, color: COLORS.text }}>
...
    const { error } = await supabase.from('seasons').insert({ team_id: team.id, name: newName.trim(), start_date: newStartDate, end_date: newEndDate });
...
      {canEdit && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16, border: `1px solid ${COLORS.border}`, background: COLORS.navyMid, borderRadius: 8, padding: 10 }}>
          <input placeholder='e.g. "Summer 26"' value={newName} onChange={e => setNewName(e.target.value)}
            style={{ padding: 8, background: COLORS.navyDark, border: `1px solid ${COLORS.border}`, borderRadius: 7, color: COLORS.text }} />
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 4 }}>Start date</div>
              <input type="date" value={newStartDate} onChange={e => setNewStartDate(e.target.value)}
                style={{ width: '100%', padding: 8, background: COLORS.navyDark, border: `1px solid ${COLORS.border}`, borderRadius: 7, color: COLORS.text }} />
            </div>
            <div style={{ flex: 1 }}>
...
            style={{ padding: '8px 16px', background: (newName.trim() && newStartDate && newEndDate) ? COLORS.gold : COLORS.navyDark, color: (newName.trim() && newStartDate && newEndDate) ? COLORS.textDark : COLORS.muted, border: 'none', borderRadius: 7, fontWeight: 'bold', cursor: (newName.trim() && newStartDate && newEndDate) ? 'pointer' : 'default' }}>
            + Add Season
          </button>
        </div>
      )}
